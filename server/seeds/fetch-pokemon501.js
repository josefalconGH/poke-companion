...
const MAX_POKEMON = parseInt(process.env.MAX_POKEMON, 10) || 1025;
...
async function fetchPokemonFrom501To1025() {
  const pokemonData = [];
  const promises = [];
  let completed = 0;

  for (let id = START_ID; id <= MAX_POKEMON; id++) {
    ...
    if (promises.length >= MAX_CONCURRENT_REQUESTS) {
      ...
      console.log(
        `Progress: ${completed}/${MAX_POKEMON - START_ID + 1} Pokémon fetched.`
      );
  ...
  fs.writeFileSync(
    "pokemon-data-501-1025.json",
    JSON.stringify(pokemonData, null, 2)
  );
  console.log(
    "Pokémon data from 501 to 1025 saved to pokemon-data-501-1025.json"
  );
}

fetchPokemonFrom501To1025();
